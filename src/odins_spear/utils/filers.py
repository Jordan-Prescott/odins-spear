...
def file_is_open(file_path: str) -> bool:
    """Checks whether a file at the specified path is already open before attempting to execute read/write operations

    Args:
        file_path (str): path to the file

    Returns:
        bool: True/False value depicting the files handle status

    Raises:
        OSError: Thrown if the specified file is already being modified
    """
...
def load_csv(file_path: str, seperator: str=",", encoding: str="utf-8", **kwargs) -> None | pandas.DataFrame:
    """Uses pandas to read in a CSV at the specified filepath
    
    Args:
        file_path (str): path to the CSV file
        seperator (str): delimiter, default ","
        encoding (str): file encoding, default "utf-8"
        **kwargs: additional arguments for pandas.read_csv 

    Returns:
        pandas.DataFrame: In memory data of parsed CSV

    Raises:
        OSFileNotFound: Thrown if the specified file cannot be located
        OSFileHandleAlreadyOpen: Thrown if the specified file is already being modified
    """
    if file_is_open(file_path):
        raise OSFileHandleAlreadyOpen()

    try:
        data_frame = pandas.read_csv(file_path, sep=seperator, encoding=encoding, **kwargs)
        return data_frame
    except FileNotFoundError as e:
        print(f"Failed To Locate File Specified | {e}")
