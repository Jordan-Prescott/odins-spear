...
def main(
    ...
    if password_type.lower() == "sip":
        logger.info("message: Generating new SIP passwords.")
        ...
        logger.info("message: Setting new SIP passwords.")
        for user in users_and_new_passwords:
            api.authentication.put_user_authentication_service(user[0], user[1])

        logger.info("message: Setting new SIP passwords complete.")
        return [
    ...
    # voicemail/ portal
        ...
        logger.error(f"message: password type {password_type} not supported")
