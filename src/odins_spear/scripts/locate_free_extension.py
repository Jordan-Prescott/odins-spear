...
def retrieve_extensions(api, service_provider_id: str, group_id: str, logger) -> list:
    ...
    logger.info(
        "message: fetching users, hunt groups, call centers, and auto attendants"
    )
    dataset = (
...
def main(
    ...
    if range_start > range_end:
        logger.error(
            f"message: range_start {range_start} is larger than range_end {range_end}"
        )
        raise OSRangeFault
    ...
    logger.info("message: filtering for useable extension")
    for extension in range(range_start, range_end + 1):
        if extension not in extensions:
            return {"extension": extension}

    logger.error(
        f"message: no useable extensions found in range {range_start} - {range_end}"
    )
